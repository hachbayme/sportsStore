...
export default function AdminProtection({ children }: AdminProtectionProps) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-900 to-gray-800">
        <Link href="/" className="absolute top-6 left-6 text-amber-400 hover:text-amber-300">
          Retour à la boutique
        </Link>
        <div className="bg-gray-800 p-8 rounded-xl shadow-2xl border border-gray-700 w-96 backdrop-blur-sm bg-opacity-90">
          <h2 className="text-2xl font-bold text-amber-400 text-center mb-4">Accès au panneau d'administration</h2>
          <form onSubmit={handleLogin}>
            <input
              type="password"
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              placeholder="Entrez le mot de passe"
              className="w-full px-4 py-3 mb-4 rounded-lg bg-gray-700 text-white"
            />
            {error && <p className="text-red-400 text-sm mb-4">{error}</p>}
            <button className="w-full py-3 bg-amber-500 rounded-lg text-white hover:bg-amber-600">
              Se connecter
            </button>
          </form>
        </div>
